...
class EditWorkerView extends StatefulWidget {
  ...
  const EditWorkerView({
    super.key,
    required this.initialWorker,
  });

  @override
  State<EditWorkerView> createState() => _EditWorkerViewState();
}

class _EditWorkerViewState extends State<EditWorkerView> {
  ...
  void _initializeFields() {
    _fullNameController.text = widget.initialWorker.fullName;
    _phoneController.text = widget.initialWorker.phone;
    _addressController.text = widget.initialWorker.address;
    _jobPositionController.text = widget.initialWorker.jobPosition;
    _salaryController.text = widget.initialWorker.monthlySalary.toStringAsFixed(2);
    _notesController.text = widget.initialWorker.notes ?? '';
    _selectedHireDate = widget.initialWorker.hireDate;
  ...
  Future<void> _handleSubmit() async {
    ...
    final request = UpdateWorkerRequest(
    ...
    final success = await _viewModel.updateWorker(widget.initialWorker.id, request);

    if (mounted) {
      if (success) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Travailleur mis à jour avec succès'),
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(_viewModel.errorMessage ?? 'Erreur lors de la mise à jour'),
            backgroundColor: AppColors.danger,
          ),
        );
      }
